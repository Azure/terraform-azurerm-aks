...
| <a name="input_log_analytics_workspace"></a> [log\_analytics\_workspace](#input\_log\_analytics\_workspace)                                                     | (Optional) Existing azurerm_log_analytics_workspace to attach azurerm_log_analytics_solution. `resource_group_name` is optional and should be used when Log Analytics Workspace is in different RG than AKS.                                       | `object({ id=string name=string resource_group_name=string })` | `null`                      |    no    |
